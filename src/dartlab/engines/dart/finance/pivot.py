...
SNAKE_ALIASES: dict[str, str] = {
    ...
    "noncontrolling_interests_equity": "equity_nci",
    "total_liabilities_and_equity": "total_equity_and_liabilities",
    "income_taxes": "income_tax_expense",
    "pretax_profit_from_continuing_operations": "profit_before_tax",
    "earnings_per_share": "basic_eps",
    "basic_earnings_per_share": "basic_eps",
    "basic_earnings_loss_per_share": "basic_eps",
    "diluted_earnings_per_share": "diluted_eps",
    "diluted_earnings_loss_per_share": "diluted_eps",
    "shortterm_borrowings": "short_term_borrowings",
    "longterm_borrowings": "long_term_borrowings",
    "debentures": "bonds",
    "cash_and_cash_equivalents": "cash_and_equivalents",
    "current_portion_of_longterm_liabilities": "current_portion_ltb",
    "paidin_capital": "issued_capital",
    "tangible_assets": "ppe",
...
def _mergeAliases(
    ...
    """SNAKE_ALIASES에 따라 동의어 시계열을 병합 (in-place)."""
    for sjDiv in series:
        keysToDelete = []
        for alias, canonical in SNAKE_ALIASES.items():
            if alias not in series[sjDiv]:
                continue

            aliasVals = series[sjDiv][alias]
            if canonical not in series[sjDiv]:
                series[sjDiv][canonical] = aliasVals
            else:
                canonVals = series[sjDiv][canonical]
                for i in range(len(canonVals)):
                    if canonVals[i] is None and i < len(aliasVals):
                        canonVals[i] = aliasVals[i]

            keysToDelete.append(alias)
